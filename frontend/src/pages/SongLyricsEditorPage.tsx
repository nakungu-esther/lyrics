    ...
    <main className="min-h-screen bg-[#090d18] text-white">
      ...
      <div className="flex min-h-[calc(100vh-65px)] flex-col xl:flex-row">
        ...
        <aside className="w-full shrink-0 bg-[#0b111e] xl:w-[390px] 2xl:w-[450px]">
          <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4"><div><h2 className="font-semibold">Lyrics</h2><p className="mt-1 text-xs text-slate-500">Luganda · AI generated</p></div><div className="flex rounded-lg border border-slate-700 p-0.5 text-xs"><button onClick={() => setSyncMode("line")} className={`rounded px-2.5 py-1.5 ${syncMode === "line" ? "bg-violet-500 text-white" : "text-slate-400"}`}>Line sync</button><button onClick={() => setSyncMode("word")} className={`rounded px-2.5 py-1.5 ${syncMode === "word" ? "bg-violet-500 text-white" : "text-slate-400"}`}>Word sync</button></div></div>
