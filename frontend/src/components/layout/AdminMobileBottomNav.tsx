...
const items: { to: string; label: string; icon: HubIconName; end?: boolean }[] = [
  ...
  { to: "/admin/artists", label: "Artists", icon: "profile" },
  { to: "/admin/songs", label: "Songs", icon: "lyrics" },
  { to: "/admin/settings", label: "More", icon: "settings" },
];

export function AdminMobileBottomNav() {
  return <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t border-slate-800 bg-[#090d18]/95 pb-[env(safe-area-inset-bottom)] backdrop-blur lg:hidden" aria-label="Admin mobile navigation">
